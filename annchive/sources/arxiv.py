...
class ArxivSource(BaseSource):
    ...
    def _parse_atom(self, xml_content: str) -> list[SourceResult]:
        ...
        try:
            ...
            # Register the namespace to avoid prefixes in output
            ET.register_namespace('atom', 'http://www.w3.org/2005/Atom')
            
            root = ET.fromstring(xml_content)
            
            # Entries are in the default namespace - find all entry elements
            # The namespace is http://www.w3.org/2005/Atom
            ns = {'ns': 'http://www.w3.org/2005/Atom'}
            entries = root.findall(".//ns:entry", ns)
            
            # If no entries with namespace, try without namespace
            if not entries:
    ...
    def _parse_entry(self, entry) -> SourceResult:
        """Parse a single arXiv entry into SourceResult."""
        title = self._extract_text(entry, "title") or "Untitled"
        author_text = self._extract_author(entry)
        description = self._extract_text(entry, "summary")
        arxiv_id = self._extract_arxiv_id(entry)
        published_str = self._extract_text(entry, "published")
    ...
    def _extract_text(self, entry, path: str) -> Optional[str]:
    ...
    def _extract_author(self, entry) -> Optional[str]:
        ...
        authors = entry.findall("author/name")
        return ", ".join(a.text for a in authors) if authors else None
    
    def _extract_arxiv_id(self, entry) -> str:
